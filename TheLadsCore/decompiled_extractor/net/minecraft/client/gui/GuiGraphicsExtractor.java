 ...
 *  net.minecraft.client.renderer.state.gui.ColoredRectangleRenderState
...
public class GuiGraphicsExtractor {
    ...
    private void innerFill(RenderPipeline renderPipeline, TextureSetup textureSetup, int x0, int y0, int x1, int y1, int color1, @Nullable Integer color2) {
        this.guiRenderState.addGuiElement((GuiElementRenderState)new ColoredRectangleRenderState(renderPipeline, textureSetup, (Matrix3x2fc)new Matrix3x2f((Matrix3x2fc)this.pose), x0, y0, x1, y1, color1, color2 != null ? color2 : color1, this.scissorStack.peek()));
    }
    ...
    public void text(Font font, @Nullable String str, int x, int y, int color, boolean dropShadow) {
        ...
        this.text(font, Language.getInstance().getVisualOrder(FormattedText.of((String)str)), x, y, color, dropShadow);
    }
    ...
    public void textWithBackdrop(Font font, Component str, int textX, int textY, int textWidth, int textColor) {
        ...
        if (backgroundColor != 0) {
            int padding = 2;
            Objects.requireNonNull(font);
            this.fill(textX - 2, textY - 2, textX + textWidth + 2, textY + 9 + 2, ARGB.multiply((int)backgroundColor, (int)textColor));
        }
    ...
    public void blitSprite(RenderPipeline renderPipeline, Identifier location, int x, int y, int width, int height, float alpha) {
        this.blitSprite(renderPipeline, location, x, y, width, height, ARGB.white((float)alpha));
    }
    ...
    private void innerBlit(RenderPipeline pipeline, GpuTextureView textureView, GpuSampler sampler, int x0, int y0, int x1, int y1, float u0, float u1, float v0, float v1, int color) {
        this.guiRenderState.addGuiElement((GuiElementRenderState)new BlitRenderState(pipeline, TextureSetup.singleTexture((GpuTextureView)textureView, (GpuSampler)sampler), new Matrix3x2f((Matrix3x2fc)this.pose), x0, y0, x1, y1, u0, u1, v0, v1, color, this.scissorStack.peek()));
    }
    ...
    private void innerTiledBlit(RenderPipeline pipeline, GpuTextureView textureView, GpuSampler sampler, int tileWidth, int tileHeight, int x0, int y0, int x1, int y1, float u0, float u1, float v0, float v1, int color) {
        this.guiRenderState.addGuiElement((GuiElementRenderState)new TiledBlitRenderState(pipeline, TextureSetup.singleTexture((GpuTextureView)textureView, (GpuSampler)sampler), new Matrix3x2f((Matrix3x2fc)this.pose), tileWidth, tileHeight, x0, y0, x1, y1, u0, u1, v0, v1, color, this.scissorStack.peek()));
    }
    ...
    public void item(ItemStack itemStack, int x, int y) {
        this.item((LivingEntity)this.minecraft.player, (Level)this.minecraft.level, itemStack, x, y, 0);
    }

    public void item(ItemStack itemStack, int x, int y, int seed) {
        this.item((LivingEntity)this.minecraft.player, (Level)this.minecraft.level, itemStack, x, y, seed);
    }
    ...
    private void item(@Nullable LivingEntity owner, @Nullable Level level, ItemStack itemStack, int x, int y, int seed) {
        ...
        this.minecraft.getItemModelResolver().updateForTopItem((ItemStackRenderState)itemStackRenderState, itemStack, ItemDisplayContext.GUI, level, (ItemOwner)owner, seed);
        try {
            ...
            CrashReport report = CrashReport.forThrowable((Throwable)t, (String)"Rendering item");
    ...
    public void fakeItem(ItemStack itemStack, int x, int y, int seed) {
        this.item(null, (Level)this.minecraft.level, itemStack, x, y, seed);
    }
    ...
    private void itemBar(ItemStack itemStack, int x, int y) {
        if (itemStack.isBarVisible()) {
            ...
            this.fill(RenderPipelines.GUI, left, top, left + itemStack.getBarWidth(), top + 1, ARGB.opaque((int)itemStack.getBarColor()));
        }
    }
    ...
    private void itemCooldown(ItemStack itemStack, int x, int y) {
        ...
        if (cooldown > 0.0f) {
            int top = y + Mth.floor((float)(16.0f * (1.0f - cooldown)));
            int bottom = top + Mth.ceil((float)(16.0f * cooldown));
    ...
    public void map(MapRenderState mapRenderState) {
        ...
        for (MapRenderState.MapDecorationRenderState decoration : mapRenderState.decorations) {
            ...
            float width = font.width((FormattedText)decoration.name);
            float f = 25.0f / width;
            Objects.requireNonNull(font);
            float scale = Mth.clamp((float)f, (float)0.0f, (float)(6.0f / 9.0f));
            this.pose.pushMatrix();
    ...
    public void entity(EntityRenderState renderState, float scale, Vector3f translation, Quaternionf rotation, @Nullable Quaternionf overrideCameraAngle, int x0, int y0, int x1, int y1) {
        renderState.lightCoords = 0xF000F0;
        this.guiRenderState.addPicturesInPictureState((PictureInPictureRenderState)new GuiEntityRenderState(renderState, translation, rotation, overrideCameraAngle, x0, y0, x1, y1, scale, this.scissorStack.peek()));
    }
    ...
    public void skin(PlayerModel playerModel, Identifier texture, float scale, float rotationX, float rotationY, float pivotY, int x0, int y0, int x1, int y1) {
        this.guiRenderState.addPicturesInPictureState((PictureInPictureRenderState)new GuiSkinRenderState(playerModel, texture, rotationX, rotationY, pivotY, x0, y0, x1, y1, scale, this.scissorStack.peek()));
    }

    public void book(BookModel bookModel, Identifier texture, float scale, float open, float flip, int x0, int y0, int x1, int y1) {
        this.guiRenderState.addPicturesInPictureState((PictureInPictureRenderState)new GuiBookModelRenderState(bookModel, texture, open, flip, x0, y0, x1, y1, scale, this.scissorStack.peek()));
    }
    ...
    public void bannerPattern(BannerFlagModel flag, DyeColor baseColor, BannerPatternLayers resultBannerPatterns, int x0, int y0, int x1, int y1) {
        this.guiRenderState.addPicturesInPictureState((PictureInPictureRenderState)new GuiBannerResultRenderState(flag, baseColor, resultBannerPatterns, x0, y0, x1, y1, this.scissorStack.peek()));
    }

    public void sign(Model.Simple signModel, float scale, WoodType woodType, int x0, int y0, int x1, int y1) {
        this.guiRenderState.addPicturesInPictureState((PictureInPictureRenderState)new GuiSignRenderState(signModel, woodType, x0, y0, x1, y1, scale, this.scissorStack.peek()));
    }

    public void profilerChart(List<ResultField> chartData, int x0, int y0, int x1, int y1) {
        this.guiRenderState.addPicturesInPictureState((PictureInPictureRenderState)new GuiProfilerChartRenderState(chartData, x0, y0, x1, y1, this.scissorStack.peek()));
    }
    ...
    public void setTooltipForNextFrame(Font font, List<Component> texts, Optional<TooltipComponent> optionalImage, int xo, int yo, @Nullable Identifier style) {
        List components = (List)texts.stream().map(Component::getVisualOrderText).map(ClientTooltipComponent::create).collect(Util.toMutableList());
        optionalImage.ifPresent(image -> components.add(components.isEmpty() ? 0 : 1, ClientTooltipComponent.create((TooltipComponent)image)));
    ...
    public void setTooltipForNextFrame(Font font, List<FormattedCharSequence> tooltip, Optional<TooltipComponent> component, ClientTooltipPositioner positioner, int xo, int yo, boolean replaceExisting, @Nullable Identifier style) {
        ...
        component.ifPresent(tooltipComponent -> components.add(components.isEmpty() ? 0 : 1, ClientTooltipComponent.create((TooltipComponent)tooltipComponent)));
    ...
    public void tooltip(Font font, List<ClientTooltipComponent> lines, int xo, int yo, ClientTooltipPositioner positioner, @Nullable Identifier style) {
        ...
        this.pose.pushMatrix();
        TooltipRenderUtil.extractTooltipBackground((GuiGraphicsExtractor)this, (int)x, (int)y, (int)w, (int)h, (Identifier)style);
        int localY = y;
        ...
        switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{HoverEvent.ShowItem.class, HoverEvent.ShowEntity.class, HoverEvent.ShowText.class}, (HoverEvent)hoverEvent2, n)) {
            ...
            case 2: {
                HoverEvent.ShowText showText = (HoverEvent.ShowText)hoverEvent2;
                {
                    ...
                    this.setTooltipForNextFrame(font, font.split((FormattedText)text, Math.max(this.guiWidth() / 2, 200)), xMouse, yMouse);
                    return;
                }
            }
        }
