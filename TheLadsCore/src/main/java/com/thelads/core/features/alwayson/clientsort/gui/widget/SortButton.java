...
extends TriggerButton {
    private static final WidgetSprites SPRITES = new WidgetSprites(Identifier.fromNamespaceAndPath("clientsort", "widget/sort"), Identifier.fromNamespaceAndPath("clientsort", "widget/sort_disabled"), Identifier.fromNamespaceAndPath("clientsort", "widget/sort_highlighted"));

    public SortButton(AbstractContainerScreen<?> screen, Container container, Slot referenceSlot, boolean referenceLeft, boolean isPlayerInv, @Nullable ClassPolicy policy, String lowestPolicyKey, Vec2i offset, Component name) {
        super(screen, container, referenceSlot, referenceLeft, isPlayerInv, SPRITES, name, policy == null ? null : policy.getKey(), lowestPolicyKey, offset, policy == null ? true : policy.offsetFromSlot(), policy == null || policy.canSort(), policy != null && policy.showSortButton(), button -> {
    ...
    public void savePolicy(@Nullable Vec2i offset, boolean offsetFromSlot, @Nullable Operation autoOp, boolean autoOpOther, Collection<Integer> slots) {
        ...
        if (policy == null) {
            Config.options().classPolicies.put(key, ClassPolicy.create(key, offset, offsetFromSlot, this.operationAllowed ? (this.active ? Policy.KEYBIND_BUTTON : Policy.KEYBIND) : Policy.NONE, Policy.KEYBIND, Policy.KEYBIND, Policy.KEYBIND, autoOp, autoOpOther, new TreeSet<Integer>(slots)));
        } else {
            Config.options().classPolicies.put(key, ClassPolicy.create(key, offset, offsetFromSlot, this.operationAllowed ? (this.active ? Policy.KEYBIND_BUTTON : Policy.KEYBIND) : Policy.NONE, policy.stackFillPolicy(), policy.matchTransferPolicy(), policy.transferPolicy(), autoOp, autoOpOther, new TreeSet<Integer>(slots)));
        }
        Config.save();
    }
}
