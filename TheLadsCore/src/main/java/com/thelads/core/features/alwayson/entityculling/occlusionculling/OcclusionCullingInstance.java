...
import com.thelads.core.features.alwayson.entityculling.occlusionculling.cache.ArrayOcclusionCache;
import com.thelads.core.features.alwayson.entityculling.occlusionculling.cache.OcclusionCache;
import com.thelads.core.features.alwayson.entityculling.occlusionculling.util.MathUtilities;
...
public class OcclusionCullingInstance {
        ...
        try {
            ...
            for (x = minX; x <= maxX; ++x) {
                ...
                for (int y = minY; y <= maxY; ++y) {
                    ...
                    visibleOnFaceY = (byte)(visibleOnFaceY | (y == minY && relY == Relative.POSITIVE ? 4 : 0));
                    visibleOnFaceY = (byte)(visibleOnFaceY | (y == maxY && relY == Relative.NEGATIVE ? 8 : 0));
                    for (int z = minZ; z <= maxZ; ++z) {
                        ...
                        visibleOnFace = (byte)(visibleOnFace | (z == minZ && relZ == Relative.POSITIVE ? 16 : 0));
                        visibleOnFace = (byte)(visibleOnFace | (z == maxZ && relZ == Relative.NEGATIVE ? 32 : 0));
    ...
    private boolean isVisible(Vec3d start, Vec3d[] targets, int size) {
        ...
        for (int v = 0; v < size; ++v) {
            ...
            if (this.allowRayChecks && this.rayIntersection(this.lastHitBlock, start, new Vec3d(relativeX, relativeY, relativeZ).normalize())) continue;
