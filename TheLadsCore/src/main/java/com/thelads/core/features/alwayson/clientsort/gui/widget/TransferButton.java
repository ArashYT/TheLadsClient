...
extends TriggerButton {
    private static final WidgetSprites SPRITES_UP = new WidgetSprites(Identifier.fromNamespaceAndPath("clientsort", "widget/transfer_up"), Identifier.fromNamespaceAndPath("clientsort", "widget/transfer_up_disabled"), Identifier.fromNamespaceAndPath("clientsort", "widget/transfer_up_highlighted"));
    private static final WidgetSprites SPRITES_DOWN = new WidgetSprites(Identifier.fromNamespaceAndPath("clientsort", "widget/transfer_down"), Identifier.fromNamespaceAndPath("clientsort", "widget/transfer_down_disabled"), Identifier.fromNamespaceAndPath("clientsort", "widget/transfer_down_highlighted"));
    ...
    public void savePolicy(@Nullable Vec2i offset, boolean offsetFromSlot, @Nullable Operation autoOp, boolean autoOpOther, Collection<Integer> slots) {
        ...
        if (policy == null) {
            Config.options().classPolicies.put(key, ClassPolicy.create(key, offset, offsetFromSlot, Policy.KEYBIND, Policy.KEYBIND, Policy.KEYBIND, this.operationAllowed ? (this.active ? Policy.KEYBIND_BUTTON : Policy.KEYBIND) : Policy.NONE, autoOp, autoOpOther, new TreeSet<Integer>(slots)));
        } else {
            Config.options().classPolicies.put(key, ClassPolicy.create(key, offset, offsetFromSlot, policy.sortPolicy(), policy.stackFillPolicy(), policy.matchTransferPolicy(), this.operationAllowed ? (this.active ? Policy.KEYBIND_BUTTON : Policy.KEYBIND) : Policy.NONE, autoOp, autoOpOther, new TreeSet<Integer>(slots)));
        }
        Config.save();
    }
}
